...
(compile 'flocking.DynaFrame)

(defn make-frame [& name]
  (let [name (or (first name) "test")]
    (flocking.DynaFrame. name)))
...
"
  ...
  ([updater-fn slider-values slider-label initial-value]
     (let [max-idx (count slider-values)
	   ...
	   slider (doto (JSlider. JSlider/HORIZONTAL 0 max-idx (int initial-value))
...
(defn example-gui []
  (let [gui (flocking.DynaFrame. "test")
...
(comment 
  ...
  (def gui (flocking.DynaFrame. "4th"))
